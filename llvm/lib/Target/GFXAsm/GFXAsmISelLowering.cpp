...
#include "GFXAsmMachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {GFXAsm::R9, GFXAsm::R10, GFXAsm::R11,
                                    GFXAsm::R12};

void GFXAsmTargetLowering::ReplaceNodeResults(SDNode *N,
                                              SmallVectorImpl<SDValue> &Results,
                                              SelectionDAG &DAG) const {
  llvm_unreachable("GFXAsmTargetLowering::ReplaceNodeResults");
}
...
#include "GFXAsmGenCallingConv.inc"
...
SDValue
GFXAsmTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_GFXAsm);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, GFXAsm::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(GFXAsmISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_GFXAsm);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const GFXAsmSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const GFXAsmTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// GFXAsm formal arguments implementation
SDValue GFXAsmTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_GFXAsm);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &GFXAsm::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    GFXAsmFunctionInfo *UFI = MF.getInfo<GFXAsmFunctionInfo>();
...
bool GFXAsmTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_GFXAsm))
    return false;
...
SDValue
GFXAsmTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool IsVarArg,
  ...
  const GFXAsmSubtarget &STI = MF.getSubtarget<GFXAsmSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_GFXAsm);
  ...
  return DAG.getNode(GFXAsmISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue GFXAsmTargetLowering::PerformDAGCombine(SDNode *N,
...
bool GFXAsmTargetLowering::isLegalAddressingMode(const DataLayout &DL,
